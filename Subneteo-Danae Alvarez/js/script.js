...
$form.addEventListener("submit", function(e) {
  ...
  try {
    ...
    $secaoResultado.insertAdjacentHTML('beforeend', criarCard('Dirección de red', rede, converterDecimalParaBinarioQuatroOctetos(rede)));

    $secaoResultado.insertAdjacentHTML('beforeend', criarCard('Dirección de Broadcast', broadcast, converterDecimalParaBinarioQuatroOctetos(broadcast)));
    
    $secaoResultado.insertAdjacentHTML('beforeend', criarCard('Cantidad de red/subred', subrede));

    $secaoResultado.insertAdjacentHTML('beforeend', criarCard('Cantidad de host por red/subred', host));
        
  } catch (error) {
    document.querySelector('body').insertAdjacentHTML('beforeend', 
    `<div class="result alert warning">
      <p>Verifique los valores introducidos</p>
    </div>`
    )
  }
});
